...
static double glOffsetX = 0.0;
static double glOffsetY = 0.0;
static double glLastCursorX = 0.0;
static double glLastCursorY = 0.0;
int pressed = 0;
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
  if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
    glfwGetCursorPos(window, &glLastCursorX, &glLastCursorY);
    pressed = 1;
  } else if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_RELEASE) {
    double x, y;
    glfwGetCursorPos(window, &x, &y);
    glOffsetX += (x - glLastCursorX);
    glOffsetY += (y - glLastCursorY);
    glLastCursorX = 0.0;
    glLastCursorY = 0.0;
    pressed = 0;
  }
...
void glfemDrawMessage(int h, int v, char *s) {
  ...
  if (h >= 0) {
    ...
    if (strlen(s) + off > 0)
      glCallLists((GLsizei)strlen(s) + off, GL_UNSIGNED_BYTE, (GLubyte *)&s[-off]);
  }
...
void glfemReshapeWindows(GLFWwindow *window, femNodes *theNodes, int w, int h) {
  ...
  double sizeX = (maxX - minX) / 1.65 / glZoom;
  double meanX = (maxX + minX) / 2.0;
  double sizeY = (maxY - minY) / 1.65 / glZoom;
  double meanY = (maxY + minY) / 2.0;

  double ratio = (GLfloat)h / (GLfloat)w;
  double size = fmax(sizeX, sizeY);

  double left, right, top, bottom, glCursorX, glCursorY;
  glfwGetCursorPos(window, &glCursorX, &glCursorY);
  meanX -= (glOffsetX + pressed * (glCursorX - glLastCursorX)) * 2 * (sizeY / fmin(w, h));
  meanY += (glOffsetY + pressed * (glCursorY - glLastCursorY)) * 2 * (sizeY / fmin(w, h));
  if (ratio > 1.0) {
    left = meanX - size;
    right = meanX + size;
